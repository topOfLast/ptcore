...
import RenderSpinner, {Spinner} from './RenderSpinner';
...
const getSymbol = (state: State) => {
	...
	if (state === 'pending') {
		return (<Text color="gray">{figures.clock}</Text>);
	}
...
const Task: FC<BaseProps & ({ state?: StateOthers; spinner: Spinner; } | { state: StateLoading; spinner: Spinner; })> = ({
	label,
	state = 'pending',
	status,
	output,
